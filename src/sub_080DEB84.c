...
void sub_080DEB84(struct OwnerDD298 *owner)
{
    ...
    switch (owner->state) {
    case 0:
        func_080955A0((void *)0x02034B20, 0x3F3F, 0x1F1F, 8);
        ...
        *(u8 *)0x0300603D = 2;
        ...
        /* fall through */
    case 1: {
        register s32 *timer asm("r2");
        register u32 lower asm("r0");
        register u32 loop_counter asm("r8");

        if (!(owner->effect_flags & 1)) {
            func_080DCDC4(owner, 2);
        }
        timer = &owner->timer;
        lower = *timer + 4;
        ...
        {
        register struct Vec3sDD298 *transformed asm("r9") =
            &frame.transformed;
        register s16 *records asm("r3");

        for (;;) {
            register s32 record_index asm("r5");
            register s16 *record_out;
            ...
            func_08093678(frame.matrix, (void *)0x087A2CEC,
                transformed);
            {
            ...
            direction >>= 8;
            {
                ...
                {
                    register u32 base asm("r3") = 0x02034B20;
                    register u32 counter asm("r1") = loop_counter;
                ...
                {
                    register u32 base asm("r1") = 0x02034B20;
                    register u32 counter asm("r2") = loop_counter;
                    ...
                    address <<= 1;
                    address += base;
                    frame.record2 = (s16 *)address;
                    {
                        register s32 owner_y asm("r0") = owner->y;
                        register s16 *record2_store asm("r3") =
                            frame.record2;
                ...
                {
                    register s16 *record1_saved asm("sl");
    ...
    case 2: {
        ...
        register u32 *state_ptr asm("r5");

        asm volatile("" : "+r"(one));
        i = one;
        state_ptr = &owner->state;
        {
            register u32 *children asm("r4") = owner->children;

            do {
                register u32 child_offset asm("r0");
                register u32 child_index asm("r1");

                asm volatile(
                    "mov %1, r8\n\t"
                    "lsl %0, %1, #2"
                    : "=r"(child_offset), "=r"(child_index)
                    :
                    : "cc");
                if (*(u32 *)((u8 *)children + child_offset) != 0) {
                    func_08094554();
                }
                i = (u8)(i + 1);
            } while (i <= 0x1F);
        ...
        }
        {
            ...
            records2[1] = x;
                ...
                }
                {
                    ...
                    masked &= saved_flags;
                    if (masked == 0) {
                        register s32 *timer_sub asm("r4") = frame.timer;
                        register s32 value asm("r0") = *timer_sub;

                        side_x = stored_x - value;
                    } else {
                        register s32 *timer_add asm("r2") = frame.timer;
            ...
            {
                register s32 *timer_tail asm("r3") = frame.timer;
    ...
    case 5: {
        ...
        asm volatile(
            ...
            : "r"(owner));
        owner->children[0] = func_080D2450(owner, 2, 0,
            first_x, first_y, 0x400, 0, 0);
        base_x = owner->x;
        ...
        y = (s16)camera_y;
        owner->children[1] =
            func_080D2450(owner, 0, 0, x, y, 0x1500, 0, 0);
        func_080D12A0(0xC, 0);
        func_080D2790(1);
        goto advance;
    }
advance:
        ...
        break;
    case 6: {
        register void *child asm("r2") = owner->children[1];

        if (child == 0) {
            func_08095114(owner);
        } else if (*(u16 *)((u8 *)child + 0x16) == 0) {
            register u32 resource asm("r0");

            {
                register u8 *table asm("r1");

                asm volatile("ldr %0, [pc, #44]" : "=r"(table));
                resource = *(u16 *)((u8 *)child + 0x14);
                resource += (u32)table;
                resource = *(u8 *)resource;
            }
            resource <<= 5;
            {
                register u8 *graphics asm("r1");

                asm volatile("ldr %0, [pc, #40]" : "=r"(graphics));
                resource += (u32)graphics;
            }
            {
                register s32 destination asm("r1");
                register s32 *camera asm("r1");

                asm volatile("ldr %0, [pc, #40]" : "=r"(camera));
                destination = *camera;
                destination <<= 5;
                {
                    register u8 *vram asm("r2");

                    asm volatile("ldr %0, [pc, #36]" : "=r"(vram));
                    destination += (u32)vram;
                }
                func_08095208(resource, destination, 0x20);
            }
        }
        break;
    }
