...
void sub_080D9CDC(void *arg0) {
    ...
    case 0: {
        ...
        asm volatile("" : "+r"(x));
        {
            register s16 *y_ptr asm("r0") = (s16 *)(owner + 0x94);
            register s32 y_offset asm("r1") = 0;
            y = y_ptr[y_offset];
        }
        ...
        }
        {
            ...
            if (masked == 0) {
                ...
                if (counter != 0) {
                    ...
                    register s32 value asm("r2") = x_base - 0x18;
                    value += (u32)(random * 0x31) >> 15;
                    x = (s16)value;
                } else {
                ...
                if (*counter_slot != 0) {
                    register u32 random asm("r0") =
                        func_080ECD5C(D_03000010);
                    register s32 value asm("r2") = y_base - 0x18;
                    value += (u32)(random * 0x31) >> 15;
                    asm volatile(
                        "lsl %1, %1, #16\n\t"
                        "asr %0, %1, #16"
