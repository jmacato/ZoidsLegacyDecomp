...
void sub_080D624C(struct OwnerD624C *arg0) {
    volatile s32 reserve0;
    volatile s32 reserve1;
    volatile s32 reserve2;
    volatile s32 reserve3;
    volatile s32 reserve4;
    register struct OwnerD624C *owner asm("r6") = arg0;
    register s32 boundary_check asm("r4") = 0;
    register char *parent asm("r8") = owner->parent;
    ...
    if (phase <= 0xF) {
        ...
        if (phase == 0) {
            owner->position = owner->x << 8;
        }
        if ((owner->flags & 0x8000) == 0) {
            phase_view = owner->phase;
            asm volatile("" : "+r"(phase_view));
            owner->position -= phase_view << 7;
        } else {
            phase_view = owner->phase;
            asm volatile("" : "+r"(phase_view));
            owner->position += phase_view << 7;
        }
        owner->x = (u32)owner->position >> 8;
        ...
        register u32 flags_copy asm("r3");
        ...
        if ((owner->phase & 1) == 0) {
            ...
            register s32 old_x asm("r2");
            register volatile s32 *outgoing asm("sp");
            register s32 type_source asm("r0") = owner->type;
            register s32 type asm("r1") = type_source + 1;
            register u32 child_direction asm("r0");
            void *created;

            old_x = owner->x;
            child_direction = 0x80;
            child_direction <<= 8;
            child_direction &= flags_copy;
            ...
            outgoing[3] = 0x80;
            outgoing[4] = 2;
            created = func_080D2660(parent, type, 0, x);
            {
                register u32 offset asm("r1") = owner->phase;
                register char *children asm("r2");
                offset -= 0x10;
