...
void sub_080D50E4(void *arg0)
{
                        ...
                        x_value = *(s16 *)(x_view + x_offset);
                        {
                            ...
                            asm volatile(
                                ...
                                : "=r"(y_view), "=r"(y_offset),
                                  "=r"(y_value)
                                : "r"(saved_y));
                            spawned = func_080D2660(owner, 5, 0,
                                x_value, y_value, 0x400, angle, priority, 0);
                        }
                    }
                    child_offset = index;
                    child_offset += 10;
    ...
    if (state == 9) {
        ...
        index = 0;
        {
            ...
            {
                register char *x_init asm("r4") = (char *)0x90;
                asm volatile("add %0, %1, %0"
                             : "+r"(x_init) : "r"(owner));
                x_ptr = x_init;
            }
            {
                register char *y_init asm("r0") = (char *)0x94;
                asm volatile("add %0, %1, %0"
                             : "+r"(y_init) : "r"(owner));
                y_ptr = y_init;
            }
...

            {
...

                {
                    ...
                    created = func_080D2450(owner, 2, 1,
                        x_value, y_value, size, zero, zero);
                }
                {
                    register char *owner_view asm("r2") = owner;
